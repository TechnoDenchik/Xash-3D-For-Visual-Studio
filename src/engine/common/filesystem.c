...
static void FS_WriteGameInfo( const char *filepath, gameinfo_t *GameInfo )
{
	...
	FS_Print( f, "// Copyright TechnoCorporation 2023\n\n\n" );
...
void FS_CreateDefaultGameInfo( const char *filename )
{
	...
	Q_strncpy( defGI.game_dll, "cl_dlls/hl.dll", sizeof( defGI.game_dll ));
...
static qboolean FS_ParseLiblistGam( const char *filename, const char *gamedir, gameinfo_t *GameInfo )
{
	...
	Q_strncpy( GameInfo->game_dll, "cl_dlls/hl.dll", sizeof( GameInfo->game_dll ));
...
static qboolean FS_CheckForGameDir( const char *gamedir )
{
	...
	if( FS_FileExists( va( "%s/system.pak", gamedir ), false ))
		return true;
...
static qboolean FS_ParseGameInfo( const char *gamedir, gameinfo_t *GameInfo )
{
	...
	Q_snprintf( default_gameinfo_path, sizeof( default_gameinfo_path ), "%s/system.txt", fs_basedir );
	Q_snprintf( gameinfo_path, sizeof( gameinfo_path ), "%s/system.txt", gamedir );
	Q_snprintf( liblist_path, sizeof( liblist_path ), "%s/system.gam", gamedir );
